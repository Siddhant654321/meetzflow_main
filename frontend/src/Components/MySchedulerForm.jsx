...
const MySchedulerForm = ({
  buttonText,
  headline,
  update,
  oldDaysArray = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"],
  oldTime = ["0:00", "23:30"],
}) => {
  ...
  const handleClick = async (e) => {
    ...
    if (inputState.schedulerName.length <= 2) {
      inputErrors = {
        ...inputErrors,
        schedulerNameError:
          "Scheduler Name should be at least 3 characters long",
      };
    } else if (inputState.schedulerName.includes(" ")) {
      inputErrors = {
        ...inputErrors,
        schedulerNameError: "Scheduler Name cannot contain any spaces",
      };
    } else {
    ...
    if (inputState.meetingTitle.length <= 2) {
      inputErrors = {
        ...inputErrors,
        meetingTitleError: "Meeting Title should be at least 3 characters long",
      };
    } else {
    ...
    if (inputState.meetingDescription.length <= 5) {
      inputErrors = {
        ...inputErrors,
        meetingDescriptionError:
          "Meeting Description should be at least 6 characters long",
      };
    } else {
    ...
    if (daysArray.length === 0) {
      inputErrors = {
        ...inputErrors,
        daysExcludedError: "Please select at least one day",
      };
    } else {
    ...
    if (Number.isNaN(inputState.daysInAdvance)) {
      inputErrors = {
        ...inputErrors,
        daysInAdvanceError: "Only Numbers are allowed",
      };
    } else if (inputState.daysInAdvance <= 0) {
      inputErrors = {
        ...inputErrors,
        daysInAdvanceError:
          "At least 1 should be entered to ensure proper functionality",
      };
    } else {
    ...
    if (
      inputErrors.schedulerNameError === "" &&
      inputErrors.meetingTitleError === "" &&
      inputErrors.meetingDescriptionError === "" &&
      inputErrors.daysExcludedError === "" &&
      inputErrors.daysInAdvanceError === ""
    ) {
      setBtn(
        <div>
          <span
      ...
      try {
        if (update) {
          await axios.patch(
            `/schedule/endpoint/${inputState.schedulerName.toLowerCase()}`,
            { ...inputState, daysArray: daysArray, timeAllowed: time },
            { withCredentials: true }
          );
        } else {
          const response = await axios.post(
            "/schedule/endpoint/new",
            {
              ...inputState,
              schedulerName: inputState.schedulerName.toLowerCase(),
              daysArray: daysArray,
              timeAllowed: time,
            },
            { withCredentials: true }
        ...
        setBtn(
          <>
            Scheduler Created <i className="bi bi-check-lg"></i>
          </>
        );
        setShowSuccess(true);
      } catch (error) {
  ...
  return (
    <div className="scheduler-form-bg">
      {showSuccess && (
        <CopyLink
          path={`/schedule/${inputState.schedulerName.toLowerCase()}`}
        />
      )}
      <AlertPopup
        body={popupError}
      ...
      />
      <form className="scheduler-form mb-3">
        <h2>
          <b>{headline}</b>
        </h2>
        <InputFields
        ...
        <InputFields
          ...
          name="daysInAdvance"
          errorState={errors}
          error="daysInAdvanceError"
          type="number"
          margin="mb-3"
          placeholder="Advance Booking Limit"
          setInputState={setInputState}
          additionalText="Days"
        />
        <div className="mb-3">
