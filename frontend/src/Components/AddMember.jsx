  ...
  const handleClick = async () => {
    try {
      ...
      await axios.patch(
        ...
        { team: teamName, memberEmail: inputState.email },
  ...
  return (
    <Modal show={showAddMember} onHide={() => setShowAddMember(false)} centered>
      ...
      <Modal.Body className="add-new-member-div">
        ...
        />
        <select
          className="form-select mb-2 time-select"
          value={memberRole}
          onChange={handleChange}
          aria-label="Member Role"
        >
          <option value="member" selected className="m-options">
            Member
          </option>
          <option value="admin" className="m-options">
            Admin
          </option>
        </select>
        ...
        <button
          ...
          onClick={handleClick}
          className="submit-btn add-new-member-btn"
          type="submit"
        >
          {btn}
        </button>
