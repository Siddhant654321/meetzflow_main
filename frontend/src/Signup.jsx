...
const Signup = () => {
    ...
    if (
      ...
      try {
        const response = await axios.post(
          `${config.backend_url}/account/endpoint/newSignUp`,
          inputState,
        ...
        setSuccessMessage(
          `An email is sent to ${inputState.email} for account verification. Check Spam folder if you don't find the email.`
        );
      } catch (error) {
        if (error.response.data.error.hasOwnProperty("keyPattern")) {
          ...
          setErrors(() => ({
            ...inputErrors,
            serverError:
              "Unable to create the account due to some server error",
          }));
        }
  ...
  return (
    <div className="m-background" style={{ margin: "10px 0px" }}>
      <h2 className="m-secondary-heading">WELCOME TO MEETZFLOW</h2>
