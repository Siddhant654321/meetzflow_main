...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("loggedIn")) {
      ...
      if (
        !localStorage.getItem("isGoogleConnected") &&
        location.pathname !== "/app/setup/" &&
        location.pathname !== "/app/setup/?success=success"
      ) {
        navigate("/app/setup/");
      }
    } else {
    ...
    <div className="app-container">
      <div className={`${show ? "body-pd" : ""}`} id="body-pd">
        <div className={`l-navbar ${show ? "show-sidebar" : ""}`} id="nav-bar">
          <nav className={"nav"}>
            <div>
              <Link to="/app" className="nav_logo">
                {" "}
                <img src={meetzflow_logo} height={30} width={30} />{" "}
                <span className="nav_logo-name">MeetzFlow</span>{" "}
              </Link>
              <div className="nav_list">
                ...
                />
                <div
                  className="nav_link collapse-div"
                  onClick={toggleShow}
                  style={{ boxSizing: "content-box" }}
                >
                  <button
                    className={`toggle-btn rounded-circle`}
                    id="toggle-btn"
                  >
                    <i
                      className={`bi bi-${
                        show ? "chevron-left" : "chevron-right"
                      } nav_icon`}
                    ></i>{" "}
                  </button>
                  <span className="nav_name collapse-txt">Collapse</span>
                </div>
              </div>
            </div>
            <SidebarItems
