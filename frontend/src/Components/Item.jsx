...
const Item = ({
  ...
  const [arrayLength, setArrayLength] = useState(() =>
    content.length + dataChanged < limit ? content.length + dataChanged : limit
  );
      ...
      const arrayData = myArray.map((element, index) => {
        if (name === "Notifications") {
            ...
            <div
              ...
              className={
                index !== myArray.length - 1
                  ? "item-container item-container-mobile item-bottom-border"
                  : "item-container item-container-mobile"
              }
            >
              ...
              <div className="view-btn view-btn-mobile">
                <button
                  onClick={() => handleNotificationClick(element.criteria)}
                >
                  View
                </button>
              </div>
          ...
          return (
            ...
            >
              <div className="item-name">
                <p>Meeting with {element.scheduledBy}</p>
              </div>
              <div className="item-time">
                <p>{formattedTime}</p>
              </div>
              ...
              <div className="view-btn view-btn-mobile">
                <button
                  onClick={() => navigate(`/app/meetings/${element._id}`)}
                >
                  View
                </button>
              </div>
          ...
          return (
            <div key={element._id} className="item-container">
              <div className="item-name">
                <p>{element.schedulerName}</p>
              </div>
              <div>
                <button
                  onClick={() => handleShowLink(element.schedulerName)}
              ...
              <div className="view-btn">
                <button
                  onClick={() =>
                    navigate(
                      `/app/meetings/scheduler/update/${element.schedulerName}`
                    )
                  }
                >
                  Edit
          ...
          return (
            <div key={element._id} className="item-container">
              ...
              {element.isAdmin && (
                <DeleteItemButton category="Teams" name={element.team} />
              )}
              <div className="view-btn">
                <button onClick={() => navigate(`/app/teams/${team}/chat`)}>
                  View
                </button>
              </div>
            </div>
  ...
  return (
    <div className={`${className} item-div-main`}>
      <div className="title">
        <p>{name}</p>
        {noNewBtn ? null : name === "Schedulers" ? (
          <CreateNewSchedulerButton />
        ) : (
          <CreateNewTeamButton content={content} />
        )}
      </div>
      <div className="content-class">
        ...
        {showSchedulerLink && (
          <CopyLink
            path={`/schedule/${schedulerName}`}
            setShowSchedulerLink={setShowSchedulerLink}
          />
        )}
        {Array.isArray(content) && content.length > limit ? (
          <div className="d-flex justify-content-center mb-3 mt-sm-3">
            {pagination ? (
              <Pagination
                limit={limit}
              ...
              <button className="view-more-btn align-middle">
                View More <i className="bi bi-arrow-right-circle"></i>
              </button>
            )}
          </div>
        ) : null}
