      ...
      <div className={`input-group ${layout} ${margin}`}>
        <div className={`form-floating`}>
          <input
            disabled={disabled}
            type={type}
            className={`form-control ${input_className}`}
            value={inputState[name]}
            onChange={changeInput}
            id={`floating${name}`}
            placeholder={placeholder}
          />
          <label className="m-label" htmlFor={`floating${name}`}>
            {placeholder}
          </label>
          {password && (
            <button
              type="button"
              className={`btn btn-white position-absolute top-50 end-0 translate-middle-y password-btn ${
                hasError ? "has-error" : ""
              }`}
