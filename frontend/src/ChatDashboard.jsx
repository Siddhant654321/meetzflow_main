...
const ChatDashboard = () => {
  ...
  const [scrollHeightBeforeNewMessages, setScrollHeightBeforeNewMessages] =
    useState(0);
  const [moreMessages, setMoreMessages] = useState(() => false);
  const [moreMessagesBtn, setMoreMessagesBtn] = useState(
  ...
  const fetchMessages = async (page, isShowMoreClicked = false) => {
    try {
      ...
      const messages = await axios.get(
        `/chat/endpoint/messages/${name}?page=${page}`,
        { withCredentials: true }
      );
      const teamData = await axios.get(`/team/endpoint/oneTeam/${teamName}`, {
        withCredentials: true,
      });
      const oldMessages = [];
      let localVar = null;
      ...
      messages?.data.forEach((message) => {
        ...
        if (localVar !== formattedDate) {
          ...
          dateArray.forEach((targettedDate) => {
            if (formattedDate === targettedDate) {
              ...
              duplicateMessageArray.forEach((messageData, index) => {
                if (
                  messageData.type.name === "ChatDate" &&
                  messageData.props.date === formattedDate
                ) {
                  indexOfDate = index;
                }
              });
          ...
          oldMessages.push(
            <ChatDate date={formattedDate} key={oldMessages.length} />
          );
        ...
        if (message.chat.hasOwnProperty("imageMessage")) {
          return oldMessages.push(
            <ChatMessages
              key={message.chat._id}
              senderName={message.chat.sentByName}
              senderTime={formatTime(chatTime)}
              sender={
                message.chat.sentByEmail === localStorage.getItem("email")
                  ? "me"
                  : "other"
              }
              image={`${config.backend_url}/chatImages/${teamData.data._id}/${message.chat.imageMessage}`}
            />
          );
        } else if (message.chat.hasOwnProperty("meetingMessage")) {
          return oldMessages.push(
            <p className="meeting-message" key={message.chat._id}>
              {message.chat.sentByEmail === localStorage.getItem("email")
                ? "You have " + message.chat.meetingMessage
                : `${message.chat.sentByName} has ${message.chat.meetingMessage}`}{" "}
              - <a href={message.chat.meetingLink}>Meeting Link</a>
            </p>
          );
        }
        return oldMessages.push(
  ...
  useLayoutEffect(() => {
    if (!moreMessagesClicked) {
      ...
      const heightDifference =
        scrollRef.current.scrollHeight - scrollHeightBeforeNewMessages;
  ...
  useEffect(() => {
    ...
    socketRef.current.on("receiveMeetingMessage", (data) => {
      ...
      if (lastMessageDate !== formattedDate) {
        ...
        setMessage((prev) => [
          ...
          <div key={message.length}>
            ...
            <p className="meeting-message">
              {`${data.from} has hosted a meeting for '${data.meetingTitle}' and it will commence on ${data.meetingTime}`}{" "}
        ...
        setMessage((prev) => [
          ...prev,
          <p key={message.length} className="meeting-message">
            {`${data.from} has hosted a meeting for '${data.meetingTitle}' and it will commence on ${data.meetingTime}`}{" "}
            - <a href={data.meetingLink}>Meeting Link</a>
          </p>,
        ]);
  ...
  const handleFileChange = async (event) => {
    ...
    imageArray.forEach((image) => {
      if (!image.type.startsWith("image/")) {
        if (!showPopup) {
          setShowPopup(true);
          setError(
            "Only images are allowed to be send. Any other file will not be send"
          );
        }
        return;
      }
    ...
    try {
      await axios.post("/chat/endpoint/newImage", data, {
        withCredentials: true,
      });
    } catch (error) {
      setShowPopup(true);
    ...
    setBtn(<i className="bi bi-images text-white"></i>);
  };
  ...
  const onSend = async () => {
    ...
    try {
      ...
      await axios.post("/chat/endpoint/newMessage", data, {
        withCredentials: true,
      });
    } catch (error) {
      setShowPopup(true);
  ...
  const handleMeetingSchedule = async (e) => {
    ...
    if (meetingData.meetingTitle.length <= 2) {
      return setErrors((prev) => ({
        meetingTitleError:
          "Meeting title should be at least 3 characters long!",
      }));
    }
    ...
    try {
      const response = await axios.post(
        "/meetings/endpoint/team/new",
        { ...meetingData, currentTime: new Date(), team: name },
        { withCredentials: true }
      );
      const chatTime = new Date();
      const formattedDate = formatDate(chatTime);
      ...
      setMessage((prev) => [
        ...prev,
        <p key={message.length} className="meeting-message">
          {`You have hosted a meeting for '${meetingData.meetingTitle}' and it will commence on ${response.data.meetingTime}`}{" "}
          - <a href={response.data.meetingLink}>Meeting Link</a>
        </p>,
      ]);
      setMeetingSuccess(() => "Meeting Successfully Scheduled!");
      setIsMeetingBtnDisabled(true);
      setMeetingBtn(<i className="bi bi-check-lg text-white"></i>);
    } catch (error) {
    ...
    <form className="schedule-meeting-form">
      <MyCalendar
        ...
        onChange={(selectedDate) =>
          setMeetingData((prev) => ({ ...prev, selectedDate }))
        }
      ...
      <div className="chat-dashboard-secondary-div">
        <div className="chat-header">
          ...
          <button onClick={() => setShowMenu(true)}>
            <i className="bi bi-three-dots-vertical text-white chat-menu-icon"></i>
          </button>
        </div>
        <AlertPopup
          body={error}
        ...
        />
        <AlertPopup
          body={meetingPopupBody}
          header="Schedule Meeting"
          showPopup={showMeetingPopup}
          setShowPopup={setShowMeetingPopup}
        />
        <MenuModal
          teamName={name}
          showMenu={showMenu}
        ...
        <div className="input-group mb-3 chat-message-form">
          ...
          <button
            disabled={isImageBtnDisabled}
            className="insert-file-chat-button"
            type="button"
            id="button-addon2"
