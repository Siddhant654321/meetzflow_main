...
const CreateNewTeamButton = ({ content }) => {
  ...
  const handleClick = async () => {
    ...
    if (newTeamName.length < 2) {
      setErrors({
        teamNameError: "Team Name Should be at least 2 characters long",
      });
    } else if (isAlready) {
  ...
  const popupBody = (
    <div className="add-new-team-div">
      ...
      <button
        ...
        onClick={handleClick}
        className="submit-btn add-new-team-btn"
        type="submit"
      >
        {btn}
      </button>
  ...
  return (
    <div>
      <button className="create-new-btn" onClick={() => setShowPopup(true)}>
        Create New
      </button>
      <AlertPopup
        header="Create Team"
