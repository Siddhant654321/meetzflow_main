...
const Signin = () => {
  ...
  const handleClick = async (e) => {
    ...
    if (localStorage.getItem("loggedIn")) {
      inputErrors = {
        ...inputErrors,
        serverError: "You are already logged in one account",
      };
    }
    ...
    if (
      ...
      try {
        const response = await axios.post(
          `${config.backend_url}/account/endpoint/login`,
          inputState,
        ...
        if (error.response.data.error === "Either email or password is wrong") {
          setErrors(() => ({
            ...inputErrors,
            serverError: "Either email or password is wrong",
          }));
        } else if (error.response.data.hasOwnProperty("emailError")) {
          setErrors(() => ({
            ...inputErrors,
            serverError: error.response.data.emailError,
          }));
        } else {
          setErrors(() => ({
            ...inputErrors,
            serverError:
              "Unable to log in to the account due to some server error",
          }));
        }
  ...
  return (
    <div className="m-background">
      <form id="join" novalidate>
        <h2 className="m-secondary-heading">Welcome Back</h2>
        <InputFields
        ...
        <Link
          to="/forgot-password"
          state={{ email: inputState.email }}
          className="forgot-password m-green-text"
