...
const SchedulerTemplate = () => {
  ...
  const fetchData = useFetchScheduler(
    schedulerName,
  ...
  const handleMeetingBooking = useHandleMeetingBooking(
  ...
  useEffect(() => {
    const timeZonesJSX = timeZoneList.map((timeZone) => (
      <option value={timeZone} key={timeZone} className="m-options">
        {timeZone}
      </option>
    ));
  ...
  useEffect(() => {
    handleMeetingBooking(
      {
        selectedDate: templateVariables.selectedDate,
        data: templateVariables,
        setData: setTemplateVariables,
      },
      false
    );
  ...
  useLayoutEffect(() => {
    if (templateVariables.timeAllowed !== "") {
      const newTimeArray = templateVariables.timeAllowed.map((time) => (
        <option value={time} key={time} className="m-options">
          {time}
        </option>
      ));
  ...
  const isTileDisabled = ({ activeStartDate, date, view }) => {
    if (templateVariables.daysArray !== "") {
      ...
      return (
        !templateVariables.daysArray.includes(daysOfWeek[date.getDay()]) &&
        view === "month"
      );
    }
  };
  ...
  const handleClick = async (e) => {
    ...
    if (templateVariables.scheduledBy.length <= 2) {
      inputErrors = {
        ...inputErrors,
        scheduledByError: "Your Name should be at least 3 characters long",
      };
    } else {
    ...
    if (!templateVariables.timeAllowed.includes(templateVariables.time)) {
      inputErrors = {
        ...inputErrors,
        timeAllowedError: "Sorry but this time slot is not available",
      };
    } else {
    ...
    if (
      inputErrors.scheduledByError === "" &&
      inputErrors.schedulerEmailError === ""
    ) {
      setBtn(
        <div>
          <span
      ...
      try {
        const response = await axios.post(
          `${config.backend_url}/meetings/endpoint/new`,
          { ...templateVariables, scheduler: schedulerName }
        );
        setPopupData({ body: response.data, header: "Scheduled" });
        setShowPopup(true);
        setBtn(
          <>
            <span>Meeting Scheduled </span> <i className="bi bi-check-lg"></i>
          </>
        );
      } catch (error) {
  ...
  return (
    <div className="m-dashboard-container py-3">
      <AlertPopup
        body={popupData.body}
      ...
      <form style={{ maxWidth: "100%", width: "100%", padding: "0px" }}>
        ...
        <div className="meeting-description">
          <p style={{ margin: "0px" }}>
            ...
            {templateVariables.meetingDescription}
          </p>
        </div>
        <MyCalendar
        ...
        <div className="meeting-details">
          <InputFields
            ...
            key="scheduledBy"
            inputState={templateVariables}
            name="scheduledBy"
            errorState={errors}
            error="scheduledByError"
            type="text"
            margin="mb-3"
            placeholder="Your Name"
            setInputState={setTemplateVariables}
          />
          <InputFields
            input_className="m-authentication-fields"
            key="schedulerEmail"
          ...
          <select
            className="form-select custom-scroll-bar mb-3 time-select"
            value={templateVariables.timeZone}
          ...
          <button
            className="m-edit-scheduler mb-3 w-100"
            style={{ width: "50%" }}
            type="submit"
